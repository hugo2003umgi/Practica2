...
#1
grafica1 = ggplot(ds, aes(x = MinTemp, y = MaxTemp))
grafica1

#2
ggplot(ds, aes(x = MinTemp, y = MaxTemp))+geom_point()

#3
grafica2 = ggplot(ds, aes(x = MinTemp, y = MaxTemp, color =RainTomorrow ))+geom_point()
grafica2

#4
grafico3 = grafica2 + ggtitle("Gráfico de dispersión tiempo mínimo frente a tiempo máximo")
grafico3
...
#1
ggplot(ds, aes(x = Temp9am, y = Pressure9am))+geom_point()

#2
ggplot(ds, aes(x = Temp9am, y = Pressure9am, shape= RainToday))+geom_point()

#3
ggplot(ds, aes(x = Temp9am, y = Pressure9am, color= RainToday))+geom_point()

# La información de la variable RainToday se observa mucho mejor con la 
# diferenciación por colores.

#4
ggplot(ds, aes(x = Temp9am, y = Pressure9am, color= RainToday))+geom_point() + ggtitle("Gráfico de dispersión temperatura frente a presion a las 9am")
...
# Incluye información de la variable ``Measure``
iris.grafico2 + facet_grid(Measure~.)
iris.grafico2 + facet_grid(.~Measure)

# Los valores de las dos variables se comparan mejor con la disposición por
# columnas. 

# Cambia ahora la capa **geom_point()** del gráfico anterior por la capa  **geom_jitter()**.
ggplot(iris.tidy, aes(x = Species, y = Value, color = Part)) + 
geom_jitter() + facet_grid(.~Measure) + ggtitle("Gráficos de dispersión")
# Con respecto a geom_point(), en geom_jitter() los puntos no se disponen sobre 
# rectas, si no que se visualizan más dispersas.
...
# Representa un gráfico de puntos...
ggplot(data=DatosEjer4, aes(x=Fecha, y= Ventas, group=Fabricante)) +
geom_point(aes(color=Fabricante))
# Si que no es válido para evaluar la evolución de las ventas de cada fabricante.

# Unimos los puntos con una linea
ggplot(data=DatosEjer4, aes(x=Fecha, y= Ventas, group=Fabricante)) +
geom_point(aes(color=Fabricante)) + geom_line(aes(color=Fabricante))

# Diferentes tipos de lineas
ggplot(data=DatosEjer4, aes(x=Fecha, y= Ventas, group=Fabricante)) +
geom_point(aes(color=Fabricante)) + geom_line(aes(color=Fabricante, linetype=Fabricante))
...
# Representa un gráfico de puntos...
ggplot(data=Experimento, aes(x=Dosis, y= Altura, group=Complemento)) +
geom_point(aes(color=Complemento)) + geom_line(aes(color=Complemento, linetype=Complemento))

# Crea otro gráfico a partir de añadir al gráfico anterior barras de error a cada punto
ggplot(data=Experimento, aes(x=Dosis, y= Altura, group=Complemento)) +
geom_point(aes(color=Complemento)) + geom_line(aes(color=Complemento, linetype=Complemento))+
geom_errorbar(aes(ymin=Altura-Error_Altura, ymax=Altura+Error_Altura), width=.1, position=position_dodge(0.05)) + theme_classic()


ggplot(data=Experimento, aes(x=Dosis, y= Altura, group=Complemento, color=Complemento)) +
geom_point() + geom_line(aes(color=Complemento))+
geom_errorbar(aes(ymin=Altura-Error_Altura, ymax=Altura+Error_Altura), width=.1, position=position_dodge(0.05)) + theme_classic()+ scale_color_brewer(palette="Reds") + labs(title="Gráfico de la variable Dosis frente a Altura",x="Dosis", y = "Altura") 
...
# Crea un gráfico en el que estén los boxplots de la variable ``MinTemp`` para 
# cada uno de los años presentes en la variable ``Year`` del dataset.
...
#En el gráfico anterior elimina la leyenda y coloca los años en el eje $x$ a 45 grados de inclinación.
...
#Añade título ("Histograma de la variable Altura"), una etiqueta en el eje $x$ ("Altura (cm)") y una etiqueta en el eje $y$ ("Frecuencia").
ggplot(data=DF, aes(x=HEIGHT)) + geom_histogram() + labs(title="Histograma de la variable Altura",x="Altura (cm)", y = "Frecuencia")

#Ajusta el rango de representación entre 155 y 195, además consigue que el histograma tenga 20 *bins*.
ggplot(data=DF, aes(x=HEIGHT)) + geom_histogram(breaks=seq(155, 195, by=2) ) + labs(title="Histograma de la variable Altura",x="Altura (cm)", y = "Frecuencia")

#Haz que las barras del histograma sean de color azul con una grado de transparencia del 50% y el contorno de color rojo.
ggplot(data=DF, aes(x=HEIGHT)) + geom_histogram(breaks=seq(155, 195, by=2), col= "red",fill="blue", alpha=0.5 ) + labs(title="Histograma de la variable Altura",x="Altura (cm)", y = "Frecuencia")

#Añade la curva de distribución estimada de color verde.geom_density(col="black") 
ggplot(data=DF, aes(x=HEIGHT)) + geom_histogram(aes(y =..density..),breaks=seq(155, 195, by=2), col= "red",fill="blue", alpha=0.5 ) + geom_density(col="green") + labs(title="Histograma de la variable Altura",x="Altura (cm)", y = "Frecuencia")

#Representa el mismo histograma del apartado anterior para cada valor de la variable ``SMOKE``, recuerda utilizar la función **facet_grid()**. facet_grid(.~)
ggplot(data=DF, aes(x=HEIGHT)) + geom_histogram(aes(y =..density..),breaks=seq(155, 195, by=2), col= "red",fill="blue", alpha=0.5 ) + geom_density(col="green") + labs(title="Histograma de la variable Altura",x="Altura (cm)", y = "Frecuencia") +
facet_grid(.~SMOKE)
